...
export function SectionDivider({ className = '' }: { className?: string }) {
  return (
    ...
    >
      <span className="h-px flex-1 max-w-[36%] bg-gradient-to-r from-transparent via-primary/25 to-primary/40" />
      <span className="mx-3 flex items-center gap-1.5">
        <span className="h-1 w-1 rotate-45 bg-primary/60" />
        <span className="h-1.5 w-1.5 rotate-45 border border-primary/70" />
        <span className="h-1 w-1 rotate-45 bg-primary/60" />
      </span>
      <span className="h-px flex-1 max-w-[36%] bg-gradient-to-l from-transparent via-primary/25 to-primary/40" />
    </div>
  );
}
